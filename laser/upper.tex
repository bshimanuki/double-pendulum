...
\begin{tikzpicture}
	...
	% serial port
	\newcommand\serialoffset{-0.4in}
	\draw (\serialoffset, 0.5in) circle (\sixthirtytworadius);
	\draw (\serialoffset, -0.5in) circle (\sixthirtytworadius);
	\node[draw,shape=rectangle,minimum width=0.25in,minimum height=0.75in,anchor=center] at (\serialoffset,0) {};
